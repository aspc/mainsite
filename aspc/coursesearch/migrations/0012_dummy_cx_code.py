...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        """
        Create a dummy cx_code for existing courses so that the
        unique constraint can be added.
        """
        for course in orm.Course.objects.all():
            course.cx_code = "(legacy) {0}".format(course.code)
            course.save()

    def backwards(self, orm):
        """
        Migrating backwards will just discard the cx_code when the field 
        is removed (not that this would ever happen)
        """
        pass
