...
try:
    white_list = set([line.rstrip('\n') for line in open('aspc/courses/lib/voc.txt')])
    black_list = set([line.rstrip('\n') for line in open('aspc/courses/lib/no_voc.txt')])
except:
    white_list, black_list = [], []


def has_vocab(phrase, vocabs):
    words = phrase.split(' ')
    for word in words:
        if word in vocabs:
            return True
...
def is_acceptable(phrase, min_char_length, max_words_length):
    ...
    if not has_vocab(phrase, white_list):
        return 0

    if has_vocab(phrase, black_list):
        return 0
